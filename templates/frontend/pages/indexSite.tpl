				...
				{iterate from=regular_journals item=journal}	
					...
					<li class="row {if $thumb}has_thumb{/if}">
						...
						{if $thumb}
							...
							<div class="thumb col-sm-2 pt-sm-3">
								<a href="{$url|escape}">
						...
						{else}
							<div class="thumb col-sm-2 pt-sm-3">
								<a href="{$url|escape}">										
									<img src="{$baseUrl}{$defaultCoverImageUrl}" alt="Journal logo"  style="object-fit: contain;">
								</a>
							</div>
						{/if}

						<div class="body col-sm-10 d-none d-sm-block">
							<h3>
								<a href="{$url|escape}" rel="bookmark">
									{$journal->getLocalizedName()}
								</a>
